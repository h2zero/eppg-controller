...
static STR_BMS_TELEMETRY_140 telemetryData;

const STR_BMS_TELEMETRY_140& getTelemetryData() {
...
void EppgBms::serialRead() {  // TODO needed?
  while (SerialBMS.available() > 0) {
    SerialBMS.read();
  }
}

void EppgBms::handleTelemetry() {
  Serial.println(F("Handling Telemetry"));
  serialRead();
  SerialBMS.readBytes(bmsData, BMS_DATA_V2_SIZE);
  parseData(bmsData);
}

// run checksum and return true if valid
// new v2
int EppgBms::checkFlectcher16(byte byteBuffer[]) {
  int fCCRC16;
  int i;
  int c0 = 0;
  int c1 = 0;
...
// for debugging
void EppgBms::printRawSentence() {
  Serial.print(F("DATA: "));
  for (int i = 0; i < BMS_DATA_SIZE; i++) {
    Serial.print(bmsData[i], HEX);
    Serial.print(F(" "));
  }
  Serial.println();
}
...
void EppgBms::parseData(byte buffer[]) {
  // if(sizeof(buffer) != 22) {
  ...
  # Bit 2: BMS Over temperature event occuring, shut down method as per configuration
  # Bit 3: BMS Overvoltage event occuring, shut down method as per configuration
  # Bit 4: BMS Undervoltage event occuring, shut down method as per configuration
